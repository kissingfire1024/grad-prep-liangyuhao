...
这是一个面向医学大模型机器遗忘（Machine Unlearning）方向的论文追踪与复现仓库。它不只给出论文列表和一句结论，而是把每篇论文拆成**可核对的复现单元**：原始假设、最小可运行实现、关键消融、指标对比、失败模式与遗留问题。

仓库目前收录 **4 篇论文**，覆盖医学 LLM 去学习方法与医学去学习基准两个方向。其中 4 篇均处于进行中，尚无端到端完成复现的条目。每个方向都附带独立的研究想法，并显式标注它所依赖的论文与当前验证状态。
